...
NVL2(컬럼, 반환값1, 반환값2)
 - 반환값1: NULL이 아닐 경우 반환할 값
 - 반환값2: NULL일 경우 반환할 값
NULLIF(비교대상1, 비교대상2): 두 개의 값이 일치하면 NULL 반환, 일치하지 않으면 비교대상1 값을 반환
...
************************************ SQL 그룹함수 ***********************************
SUM(숫자타입 컬럼):  해당 컬럼값의 총 합계 반환
AVG(숫자타입): 해당 컬럼값의 평균값을 구해서 반환
MIN(여러타입): 해당 컬럼값 중 가장 작은 값 구해서 반환
MAX(여러타입): 해당 컬럼값 중 가장 큰 값 구해서 반환
COUNT(*|컬럼|DISTINCT컬럼): 조회된 행 개수를 세서 반환
      - *: 조회된 결과의 모든 행 개수를 세서 반환
      - 컬럼: 컬럼에서 NULL이 아닌 행을 세서 반환
      - DISTINCT 컬럼: 중복 제거 후 NULL이 아닌 값 세서 반환
